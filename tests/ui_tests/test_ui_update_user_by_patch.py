"""Тест кейс для UI запроса update user через patch"""
import allure
import pytest
...
@allure.suite("ui_update_user_by_patch")
@allure.title("Запрос на обновление данных пользователя через patch")
@pytest.mark.parametrize(
    ("request_name", "user_id", "request_parameters"),
    load_data("update_user_data", "ui_data_patch"),
)
def test_ui_update_user_by_patch_with_parameters(
    ...
    with allure.step("Выполняем api запрос"):
        api_status_code, api_response_json = reqres_api.reqres_update_by_patch(
    ...
    with allure.step("Проверяем, что response api и ui совпадают"):
        ...
        # ui_response_json и api_response_json для запроса update user через patch не сравнить,
