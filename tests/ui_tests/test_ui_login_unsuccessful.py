"""Тест кейс для UI запроса login unsuccessful"""
import allure
import pytest
...
@allure.suite("ui_login_unsuccessful")
@allure.title("Запрос на авторизацию с невалидными параметрами")
@pytest.mark.parametrize(
    ("request_name", "request_parameters"), load_data("login_data", "ui_not_valid_data")
)
def test_ui_login_unsuccessful_parameters(
